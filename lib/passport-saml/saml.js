...
SAML.prototype.verifyLogoutResponse = function (doc) {
  return Q.fcall(() => {
    ...
    let secondLevelStatus = doc.LogoutResponse.Status[0].StatusCode[0].StatusCode[0];
    if (secondLevelStatus && secondLevelStatus.$.Value === "urn:oasis:names:tc:SAML:2.0:status:PartialLogout") {
      throw 'Bad status code: ' + secondLevelStatus.$.Value;
    }
