...
logger = require("/opt/gluu/node/passport/server/utils/logging");
...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, isHttpPostBinding, callback) {
  ...
  var forceAuthn = this.options.forceAuthn || true;
  var allowCreate = this.options.allowCreate || true;
  ...
  .then(() => {
    ...
    if (req.params.samlissuer && req.params.samlissuer.length > 6) {
      // First setup the issuer as the provider issuer
      var issuerBuffer = new Buffer(req.params.samlissuer, 'base64');
      var decodedProviderIssuer =  issuerBuffer.toString('ascii');
      request['samlp:AuthnRequest']['saml:Issuer']['#text'] = decodedProviderIssuer;

      // Then adjust the settings to authenticate passively and not create a mapping (if supported)
      forceAuthn = false;
      isPassive = true;
      allowCreate = false;
    }
...
SAML.prototype.generateLogoutRequest = function (req) {
  ...
  if (this.options.encryptionCert) {
    var cert = this.certToPEM(this.options.encryptionCert);
    var xmlencOptions = {
      pem: cert,
    ...
    return Q.ninvoke(xmlenc, 'encrypt', xmlbuilder.create(nameId).end(), xmlencOptions)
      .then( encryptedXml => {
        logger.log2('debug', 'Encrypted XML: ' + encryptedXml);
        let encryptedData = xmlbuilder2.create(encryptedXml).end({ format: "object" });
        logger.log2('debug', 'Encrypted Data: ' + encryptedData);
        ...
        logger.log2('debug', 'New request: ' + xmlbuilder.create(request).end());
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  .then(certs => {
    ...
    if (encryptedAssertions.length == 1) {
      ...
      var xmlencOptions = { key: this.options.decryptionPvk, warnInsecureAlgorithm: false };
...
SAML.prototype.verifyLogoutResponse = function (doc) {
  return Q.fcall(() => {
    ...
    let secondLevelStatus = doc.LogoutResponse.Status[0].StatusCode[0].StatusCode;
